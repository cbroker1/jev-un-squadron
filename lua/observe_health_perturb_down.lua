...
client.unpause()
local root="C:\\Users\\Carl-MainRig\\Projects\\jev-un-squadron\\evidence\\health_observation\\perturb_down_"..os.time()
...
for i=1,900 do
  if i<=120 then joypad.set({Down=true},1) else joypad.set({},1) end
  emu.frameadvance()
  if i%2==0 then
    for a=0,0x1FFFF do
