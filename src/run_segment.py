...
def main():
    ...
    manifest={"run_id":run.name,"mode":args.mode,"policy":args.policy,"local_started":dt.datetime.now().astimezone().isoformat(),
        ...
        "lua_sha256":sha256(ROOT / "lua/main.lua"),"controller_sha256":sha256(ROOT / "src/play_segment.py"),
        "combat_sha256":sha256(ROOT / "src/brain/combat.py"),"policy_sha256":sha256(ROOT / "src/brain/decisions.py"),
    ...
    print(f"RUN {label} ({run.name}): {args.mode.upper()}, maximum Jev attempts {manifest['max_jev_attempts']}. Ctrl+C or launchers\\stop_segment.bat stops.",flush=True)
    try:
        with (run / "emulator_stdout.txt").open("w") as out,(run / "emulator_stderr.txt").open("w") as err,(run / "console.txt").open("w") as console:
            ...
            worker=subprocess.Popen([sys.executable,"-u",str(ROOT / "src/play_segment.py"),"--output",str(run),"--mode",args.mode,
                "--policy",args.policy,
