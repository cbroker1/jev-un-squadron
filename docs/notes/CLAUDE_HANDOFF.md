...
```powershell
launchers\dashboard.bat                       # http://127.0.0.1:8770, follows runs by itself
...
python src/run_segment.py ... --load-slot 3 --expected-start-frame 24121 --warmup 12   # boss practice, a minute an attempt
python src/runs_table.py --last 12 --full-level-only     # units, share, hits, change under test
python src/benchmark.py --recent 3 --baseline 12         # per-segment regression check
python src/replay_run.py --best-boss                     # watch a run back
python src/build_danger_map.py; python src/build_terrain_map.py
python -m unittest discover -s tests -t tests   # 97 offline tests
```
