...
def main():
    ...
    manifest = {"run_id": run.name, "jev_requests": 0, "mode": args.mode, "requested_speed": 50,
                ...
                "lua_sha256": sha256(script), "bridge_sha256": sha256(ROOT / "bridge.py")}
    env = os.environ.copy()
    ...
    try:
        with (run / "emulator_stdout.txt").open("w") as out, (run / "emulator_stderr.txt").open("w") as err:
            ...
            with (run / "bridge_console.txt").open("w") as output:
                if args.mode == "brain-preview":
                    command = [sys.executable, "-u", str(ROOT / "observe_brain.py"), "--output", str(run), "--frames", "900"]
                else:
                    command = [sys.executable, "-u", str(ROOT / "bridge.py")]
    ...
    if args.mode == "brain-preview":
        ...
        reference = ROOT / "hazard_observation/probe_20260919-114543-d95c1f_neutral"
