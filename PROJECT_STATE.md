...
**Start with [CLAUDE_HANDOFF.md](CLAUDE_HANDOFF.md).** Evidence: [object-table report](hazard_observation/OBJECT_TYPES_2026-09-19.md). Level 1 is **not finished**; the segment ends on its frame budget. About 5,700 paid attempts this session (free tier, approved).
...
- **56 offline tests pass.** New tools: `survey_objects.py`, `render_object_types.ps1`, `build_terrain_map.py`, `build_terrain_profile.py` (does not work; colour cannot separate ground from background art), `probe_hazards.py --modes replay`.
...
**Single next experiment:** repeat `run_jev_segment.bat` several times for a distribution, rebuilding `terrain_map.json` between batches, and check whether terrain hits keep falling. Then extend past frame 22463 toward the boss, surveying new object routines from a replay capture first.

## Earlier checkpoint - 2026-09-19 evening, object table, power-ups and tanks (Claude session, historical)

**Start with [CLAUDE_HANDOFF.md](CLAUDE_HANDOFF.md).** Latest evidence: [object-table report](hazard_observation/OBJECT_TYPES_2026-09-19.md). Level 1 is **not solved**. Total paid attempts: **170**.
...
- **Tools:**
  ...
  - `survey_objects.py` and `render_object_types.ps1`.
...
- `run_jev_segment.bat` now runs 900 frames with a cap of 45. **52 offline tests pass.**
...
**Single next experiment:** repeat `run_jev_segment.bat` 2-3 times to see whether 0 hits holds. Then add a code-computed safe tank line, for example "level with a tank at least N px ahead", so tank kills return without collisions. Then extend the window past frame 21563.

## Earlier checkpoint - 2026-09-19, pause-and-step (Claude session, historical)

**Start with [CLAUDE_HANDOFF.md](CLAUDE_HANDOFF.md).** Latest evidence: [pause-and-step report](hazard_observation/PAUSE_AND_STEP_2026-09-19.md). This supersedes the aircraft-slot checkpoint below. Level 1 is **not solved**. Carl approved paid requests (free tier). Total paid attempts so far: **68** (3 earlier, 65 this session).
...
- `run_jev_segment.bat` now runs the stepped + firing-prelude configuration (max 14 requests, 420 frames).
...
- The first regular-bridge dry-run check (`bridge-check-20260919-140406-33db6b`) exposed a latent `bridge.py` crash on a torn state read after the final acknowledgement.
  ...
  - `launch.bat` is otherwise unchanged.
- **45 offline tests pass.** New or extended tools: `analyze_aircraft_slot.py`, `render_run_track.ps1`, `replay_brain.py --combat`, and single-base labels in `render_projectile_evidence.ps1`.
...
**Single next experiment (paid, only with Carl's explicit go-ahead):** one bounded live combat segment, `run_jev_segment.bat` (at most 5 Jev requests, same save, 50% speed), with the 7-aircraft-slot observation. Compare it against the new baseline and record:
...
- That run stopped at frame 20732 when player X became 15, outside the tested controllable range 16..239. A zero-API replay `hazard_observation/probe_20260919-132216-1dd64b_pulse8` reproduced the same transition and captured a later explosion sequence. Do not widen the control bounds to suppress the guard or turn this into a health/death RAM mapping.
...
Evidence at that earlier point: [brain adoption report](hazard_observation/BRAIN_ADOPTION_2026-09-19.md). All tests in this earlier checkpoint used zero Jev requests; the later three paid attempts are documented above.
...
- `launch.bat` still offers D/L with the five-second countdown; the reusable local credential file is unchanged. The normal live prompt remains explicitly **fixed-target-70 calibration**. No Jev request has consumed the new projectile summaries, and no new threat-aware Jev gameplay is claimed.
...
- Added and ran `run_live_smoke.bat`: it automatically starts BizHawk, loads `main.lua`, performs five live Jev calls from the local credential file, releases controls, and closes the emulator. The run logged five applied `up + Y-fire` actions with 223–495 ms latency.
