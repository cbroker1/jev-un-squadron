...
$lua=Join-Path $root 'lua\observe_health.lua'
...
$p.WaitForExit()
# EmuHawk can leave a child UI process briefly after the launcher process exits.
# Do not let the next observation overlap it.
for ($n=0; $n -lt 30; $n++) {
  if (-not (Get-Process EmuHawk -ErrorAction SilentlyContinue)) { break }
